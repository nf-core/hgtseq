...
workflow PREPARE_READS {
    ...
    fasta_meta = Channel.value(fasta).map{ it -> [[id:it[0].getBaseName()], it] }
    ...
    if (aligner == "bwa-mem") {
        ...
        bwaindex      = params.fasta_meta ? params.bwaindex      ? Channel.fromPath(params.bwaindex).collect()      : BWAMEM1_INDEX.out.index : []
        ...
        bwamem2index  = params.fasta_meta ? params.bwamem2index  ? Channel.fromPath(params.bwamem2index).collect()  : BWAMEM2_INDEX.out.index : []
